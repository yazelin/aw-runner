...
echo -e "${BOLD}"
cat << 'EOF'
   __ _      __ __ _ _  _ _ _  _ _  ___ _ _
  / _` \ \ /\ / /  _` | || | ' \ | ' \/ _ \ '_|
  \__,_|\_/\_/\__,_|\_,_|_||_|_||_\___/_|
EOF
echo -e "${RESET}"
echo -e "  長期運行的 GitHub Actions Runner"
echo -e "  Cloudflare Tunnel + FastAPI + gh copilot + Telegram Bot\n"
...
[[ -f "$REPO_ROOT/server/main.py" ]]          || error "找不到 server/main.py，請在 aw-runner repo 根目錄的 scripts/ 執行此腳本"
[[ -f "$REPO_ROOT/worker/package.json" ]]     || error "找不到 worker/package.json"
[[ -f "$REPO_ROOT/.github/workflows/runner.yml" ]] || error "找不到 .github/workflows/runner.yml"
...
if [[ -n "$REMOTE_URL" ]]; then
  ...
  echo    "  輸入 repo 名稱（直接 Enter 使用預設 'aw-runner'）："
  read -r REPO_NAME
  REPO_NAME="${REPO_NAME:-aw-runner}"
...
if [[ -z "$WORKER_URL" ]]; then
  ...
  ask WORKER_URL "請手動輸入 Worker URL（例：https://aw-runner-worker.xxx.workers.dev）："
fi
...
gh workflow run runner.yml --repo "$REPO_SLUG"
...
echo -e "  2. 傳訊息給你的 Telegram bot，等待 gh copilot 回覆"
echo ""
