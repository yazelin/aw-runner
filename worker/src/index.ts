...
async function getActiveRunner(env: Env): Promise<string | null> {
  ...
  // Race health checks — first to respond wins
  ...
  // Return first healthy one
  for (const r of results) {
    if (r) return r;
  }
  // All unhealthy — return first URL as fallback (let caller handle error)
  return urls[0];
}
